---
name: dev-browser
description: Use for interactive exploration and stateful workflows - discovering unknown page layouts via ARIA snapshots, multi-step flows where login/cookies must persist, iterative debugging. Browser stays alive between scripts. Best when you need to explore incrementally or maintain session state. Prefer playwright-skill instead for complete test suites, CI/CD pipelines, or stateless one-shot scripts.
---
...
Take screenshots when you need to visually inspect the page. **Always use `/tmp/claude/` for screenshots** to avoid accumulating files in the skill directory:

```typescript
await page.screenshot({ path: "/tmp/claude/screenshot.png" });
await page.screenshot({ path: "/tmp/claude/full.png", fullPage: true });
```
...
await page.screenshot({ path: "/tmp/claude/debug.png" });
console.log({
  url: page.url(),
